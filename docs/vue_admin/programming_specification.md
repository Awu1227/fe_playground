...
在前面我们通过 ESLint 与 Prettier 配合解决代码格式问题，但是这仅仅解决了本地代码格式问题，并且需要**手动在 VSCode 中配置自动保存**才可以。那么这样就存在一个问题，要是有人忘记配置这个东西了怎么办？他把代码写的乱七八糟的，直接就提交了怎么办呢？

所以我们就需要有一种方式来规避这种风险。

那么想要完成这么一个操作就需要使用`husky`配合 eslint 才可以实现。

我们期望通过**huksy 监测 pre-commit 钩子，在该钩子下执行 npx eslint --ext .js,.vue src**指令来去进行相关检测：

1. 执行`npx husky add .husky/precommit "npx eslint --ext .js,.vue src"`添加 commit 时的 hook

2. 该操作会生成对应文件 _pre-commit_
...
到这里，我们已经通过 pre-commit 检测到了代码的提交规范问题。

但是如何需要修改的地方很多，那么必然会影响工作效率，我们需要**自动**修复对应的问题，并且完成提交。我们可以通过**lint-staged**实现。

### lint-staged 自动修复格式错误

在上一小节，我们主要遇到两个问题

- 我们只修改个别的文件，没有必要检测所有的文件代码格式

- 它只能给我们提示出对应的错误，我们还需要手动的进行代码修改

我们想要处理这个问题，就需要使用另外一个插件[lint-staged](https://github.com/okonet/lint-staged)
...
3. 如上配置，每次它只会在你本地 commit 之前，校验你提交的内容是否符合你本地配置的 eslint 规则，校验会出现两种结果：
   - 如果符合规则：则会提交成功
   - 如果不符合规则，它会自动执行 eslint --fix 尝试帮你自动修复，如果修复成功则会帮你把修复好的代码提交，如果失败，则会提示你错误，让你把这个错误修复好之后才能允许你提交代码。
